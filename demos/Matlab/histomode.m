...
% Demo for access to TimeHarp 2600 Hardware via TH260LIB v 3.2.
...
REQLIBVER   =     '3.2';	 % this is the lib version this program expects
MAXDEVNUM   =         4;
...
if (~libisloaded('TH260lib'))    
    ...
    loadlibrary('/usr/local/lib64/th260/th260lib.so', 'th260lib.h', 'alias', 'TH260lib'); 
else
...
% In this demo we will use the first TimeHarp device we found, i.e. dev(1).
% If you have nultiple TimeHarp devices you could also check for a specific 
...
[ret] = calllib('TH260lib', 'TH260_Initialize', dev(1), MODE_HIST); 
if(ret<0)
...
[ret, HistLen] = calllib('TH260lib', 'TH260_SetHistoLen', dev(1), MAXLENCODE, HistLenPtr);
if (ret<0)
    fprintf('\nTH260_SetHistoLen error %s. Aborted.\n', geterrorstring(ret));
    closedev;
    return;
end;
...
ret = calllib('TH260lib', 'TH260_ClearHistMem', dev(1));    
if (ret<0)
    fprintf('\nTH260_ClearHistMem error %s. Aborted.\n', geterrorstring(ret));
    closedev;
    return;
end;
...
for i=0:NumInpChannels-1  
    ...
    [ret,countsbuffer(i+1,:)] = calllib('TH260lib', 'TH260_GetHistogram', dev(1), bufferptr, i, 0); 
    if (ret<0)
        fprintf('\nTH260_GetHistogram error %s. Aborted.\n', geterrorstring(ret));
        closedev;
        return;
    end;
end;
...
timeaxis = (0:MAXHISTBINS-1) * Resolution/1000;
plot(timeaxis,countsbuffer')
xlabel('time/ns');
ylabel('counts');
